...
export default {
	html: function(str, mushLog = false) {
		var buf = Array.from(new TextEncoder().encode(str));
		var len = buf.length;
		var ret = [];
		var offset = 0;
		var state = {};
		...
		if(!mushLog) {
			ret = ret.concat(Array.from(new TextEncoder().encode(htmlForState(state))));
		}
		do { try {
			// Read next byte
			var byte = buf[offset];
			...
			if(byte == "\u001b".codePointAt(0)) {
				...
				if(offset + 1 < len && buf[offset + 1] == "[".codePointAt(0)) {
					// Jump past the CSI
					offset += 2;
					var command = "";
					...
					do {
						var char = String.fromCodePoint(buf[offset]);
						command += char;
					...
					if(!fromReset || !mushLog) {
						ret = ret.concat(Array.from(new TextEncoder().encode("</span>")));
					}
					if(!toReset || !mushLog) {
						ret = ret.concat(Array.from(new TextEncoder().encode(htmlForState(state))));
					}
				...
				ret = ret.concat(Array.from(new TextEncoder().encode("&amp;")));
				++offset;
				continue;
			} else if (byte == "<".codePointAt(0)) {
				ret = ret.concat(Array.from(new TextEncoder().encode("&lt;")));
